...
sybil_bootstrap_ips_for_node() {
    local NODE_NUM=$1
    local IPS=()
    local i
    for i in "${!NODE_IPS_ARRAY[@]}"; do
        IPS+=("${NODE_IPS_ARRAY[$i]}:9651")
    done
    for i in "${!NODE_IPS_ARRAY[@]}"; do
...
start_l1_nodes() {
    ...
    local VALIDATOR_KEY_INDEX=$NODE_NUM
    local L1_BOOTSTRAP_IPS="${BOOTSTRAP_NODE_IP}:9651"
    local L1_BOOTSTRAP_IDS="$BOOTSTRAP_ID"
    local SYBIL_MODE=0
    local START_RPC_NODE=1

    if is_truthy "$SYBIL_ENABLED_LOCAL"; then
        VALIDATOR_KEY_INDEX=$((L1_VALIDATOR_START_INDEX + NODE_NUM - 1))
        L1_BOOTSTRAP_IPS=$(sybil_bootstrap_ips_for_node "$NODE_NUM")
        L1_BOOTSTRAP_IDS=$(sybil_bootstrap_ids_for_node "$NODE_NUM")
        SYBIL_MODE=1
        START_RPC_NODE=0
    fi

    if [ "$SYBIL_MODE" -eq 1 ]; then
        echo "  Starting L1 validator on $NODE_IP (staking/l1/$VALIDATOR_KEY_INDEX, sybil protection enabled)..."
    else
...
if [ "$START_RPC_NODE" = "1" ]; then
    ...
    echo "  L1 validator node started on $NODE_IP"
fi
EOF
