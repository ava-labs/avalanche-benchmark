...
identity_index="$(l1_identity_index_for_node_index "$node_index")"
...
echo "Preparing l1-$node_number on $node_host"
...
copy_paths_to_host_dir "$node_host" "$node_data_dir/staking" \
  "$repo_root/staking/l1/$identity_index/signer.key" \
  "$repo_root/staking/l1/$identity_index/staker.crt" \
  "$repo_root/staking/l1/$identity_index/staker.key"
...
disown || true
echo "started l1-$node_number pid=\$! rpc=http://$node_host:$l1_http_port/ext/bc/$l1_chain_id/rpc log=$node_data_dir/stdout.log"
SCRIPT

wait_l1_ready "$node_host" "$expected_node_id"

echo "l1-$node_number identity=l1/$identity_index ready: http://$node_host:$l1_http_port/ext/bc/$l1_chain_id/rpc"
