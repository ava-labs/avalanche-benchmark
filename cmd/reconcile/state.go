	...
	for _, r := range raw {
		if r.Key != 0 {
			return nil, fmt.Errorf(
				"%s is in the old key-swap format; its subnet predates C-chain managed weights and cannot be managed by this version — redeploy fresh (deploy.sh) or remove the state file",
				path)
		}
	}
