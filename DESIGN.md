...
- `l1 destroy` disables main validators first and management validators last. It verifies the funding key controls deactivation and receives remaining balances before submitting anything. It can continue while any balance remains, then records `DESTROYED=true` only after all disables succeed. This irreversible local marker makes zero active validators a terminal state and lets commands fail before network access rather than producing successful no-ops or stale output. `l1 address` remains available before creation because an imported key must be funded first.
