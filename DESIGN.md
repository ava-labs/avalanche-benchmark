...
- `fleet place <identity> <node>`: set a lettered validator identity's location on a numbered node, for example `fleet place a 5`. It is a SWAP, not a drop: whatever validator identity currently sits on the target node rides back to the moved identity's old node. Motivation: we always have exactly as many validator identities as validator nodes; a swap preserves that bijection automatically (nothing orphaned, nothing duplicated), which makes equivocation STRUCTURALLY inexpressible rather than merely checked. It first stops both nodes and verifies both services are inactive. It then atomically updates `deployment/placement.json` and starts both through the normal `start` path, which pushes the newly assigned keys. If either stop fails, placement and remote keys remain unchanged. If a later start fails, both intended assignments remain explicit in `placement.json` and rerunning `start` converges them. Weight-agnostic.
