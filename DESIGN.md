...
- Configuration comes from `.env`, which every command explicitly loads. `NETWORK` is `fuji` or `mainnet`; Fuji is always called Fuji, never "testnet". `PCHAIN_API` is required and explicit. `MANAGER_COMMITTEE` is required and must be `1` or `4`; the shipped example selects `1`. `FUNDING_PRIVATE_KEY` contains the raw 32-byte secp256k1 private key as 64 hex characters with no `0x` prefix. There is no key-file setting.
...
- Warp admission uses the P-chain preferred block's ACP-181 epoch height, which can predate a freshly converted management L1 even while current-height reads show it. On the exact empty-set rejection, `set-weight` visibly submits one no-op P-chain `BaseTx` so that context can advance, reports the original failure, and requires a manual rerun. It never hides the failure in an automatic retry loop.
