...
- The address book lists only the machines the fleet is MEANT TO BE UP, and every lifecycle command re-renders it fleet-wide. Motivation: alpha = count/2 + 1 is computed over the LIST, not over who answers, so naming a machine that is down raises the bar without adding anybody who can clear it. Twelve entries with six down leaves five reachable against an alpha of six and no node can state-sync; five entries all reachable needs three and works. Intent is systemd's enabled flag, the same single up/down truth the rest of the kit uses, so `stop` and `destroy` record it for free; a machine that does not answer counts as DOWN, because a lost site never gets to record its intent and that is the case this exists for. `deploy` is exempt and deliberately so: it is declarative, it installs and brings up the machines it is given, so it renders the whole inventory and does not consult liveness at all. Narrowing the book to what is live is a lifecycle concern. An empty book drops the override entirely and falls back to the stake-weighted validator set, which is a fine default.
- The refresh must reach RUNNING machines, not just the one being started. The systemd unit is `Restart=on-failure`, so a crash reloads `node.json` with the kit uninvolved: a machine left holding a book full of dead peers comes back unable to state-sync with nobody having typed a command. This is why the push is fleet-wide rather than rendered once at start.
- Accepted consequence: `node.json` is now a function of `nodes.ini` + `placement.json` + fleet-wide systemd intent, and that third input lives on the machines rather than in a file on control. `fleet status` therefore reports the intended-up count, so the book a machine holds can be explained without reading `node.json` on every box. The pushes run in parallel (`phaseParallel`), which is why touching twelve machines instead of one costs about two extra seconds rather than twelve times as long.
...
- Those values replace the previous k 20 / alpha 11 / alpha 11 / beta 12 (measured 2026-07-31, see `CONSENSUS-TUNING.md`). Alpha 11/11 was demonstrated to FORK in steady state: nodes 4, 6 and 10 finalized a different block at height 814087 than the other nine, and throughput metrics showed nothing because only 2000 of 305000 weight had diverged. The tuning dial is the alpha-confidence/k RATIO against the stake that stays online in the drill, not the raw numbers. With one of three heavy validators down that is 205000/305000 = 0.6721, and 0.633 is the tightest ratio that still clears 3000 TPS (0.65 measured 2960, 0.667 measured 1870 with stalls).
- The two alphas are tuned in OPPOSITE directions. Alpha confidence is the threshold to accrue confidence toward finalization, so higher is safer. Alpha preference is the threshold to FLIP preference, so LOWER is safer: it makes nodes snap to the emerging plurality and collapses the metastable ~50/50 sibling split that produces forks, where a high value keeps the split alive. Alpha preference therefore sits at its legal floor, which `Parameters.Verify()` fixes at `K/2 < AlphaPreference`, i.e. 31 at k 60. Alpha 11/11 was the worst arrangement available: one marginal 55% poll both flipped preference and counted toward finalization.
- k 60 rather than k 30 at the same ratio: it puts alpha preference at a lower ratio (31/60 = 0.517 vs 16/30 = 0.533) and halves sampling variance. Raising k is nearly free on a small roster because `sendQuery` samples k slots into a `bag` for vote accounting but sends to `set.Of(vdrIDs...)`, a DEDUPED set, so one poll sends at most one message per distinct validator regardless of k. Measured: tripling k from 20 to 60 raised chits per accepted block only 1.39x and REDUCED accept latency (20.8ms to 17.1ms).
- Two distinct connected-stake thresholds exist and must not be conflated. The one that stops consensus is in `sendQuery`: `abortDueToInsufficientConnectedStake` drops the query outright when `ConnectedValidators.ConnectedPercent() < AlphaConfidence/K`, logged only at Debug. Separately `MinPercentConnectedHealthy()` = `0.8*aC/k + 0.2` is referenced only in `snow/networking/handler/health.go` and merely makes a node REPORT unhealthy; at 38/60 it demands 0.707 against 0.6721 connected, so unhealthy is expected during a one-heavy-down drill while consensus works fine. A single-DC survivor must clear BOTH the 75% bootstrap gate and aC/k to poll.
