...
- `l1 destroy` disables main validators first and management validators last. It verifies the funding key controls deactivation and receives remaining balances before submitting anything. It can continue while any balance remains. There is no local destroyed flag: height-consistent P-Chain state is the lifecycle truth, and zero active validators means the chain is destroyed. Commands then fail rather than producing successful no-ops or stale output. `l1 address` remains available before creation because an imported key must be funded first.
