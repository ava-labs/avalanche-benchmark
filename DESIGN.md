...
- A freshly converted management L1 can be absent from the P-chain's safe Warp-verification height for up to 30 seconds. On the resulting empty-validator-set rejection, `set-weight` reports the management L1's on-chain `StartTime` in JST and a retry instruction. P-chain state remains the only lifecycle truth.
